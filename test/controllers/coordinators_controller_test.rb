...
class CoordinatorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create coordinator" do
    assert_difference('Coordinator.count') do
      post coordinators_url, params: { coordinator: { academic_degree: @coordinator.academic_degree, address: @coordinator.address, ci: @coordinator.ci, department: @coordinator.department, email: @coordinator.email, lastname: @coordinator.lastname, name: @coordinator.name, phone: @coordinator.phone, position: @coordinator.position, registration_date: @coordinator.registration_date, specialty: @coordinator.specialty } }
    end
  ...
  test "should update coordinator" do
    patch coordinator_url(@coordinator), params: { coordinator: { academic_degree: @coordinator.academic_degree, address: @coordinator.address, ci: @coordinator.ci, department: @coordinator.department, email: @coordinator.email, lastname: @coordinator.lastname, name: @coordinator.name, phone: @coordinator.phone, position: @coordinator.position, registration_date: @coordinator.registration_date, specialty: @coordinator.specialty } }
